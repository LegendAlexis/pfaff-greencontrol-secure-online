    ...
    <main className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="mx-auto max-w-4xl p-5 md:p-8">
        <header>
          ...
          <p className="mt-2 text-zinc-400">E-Mail-Warnungen können jederzeit zentral ein- oder ausgeschaltet werden.</p>
        </header>

        {mailMessage && (
          <div className={`mt-6 rounded-xl border p-4 font-bold ${mailMessage.ok ? "border-emerald-600 bg-emerald-950/30 text-emerald-200" : "border-red-700 bg-red-950/30 text-red-200"}`}>
            {mailMessage.text}
          </div>
        )}

        <section className={`mt-7 rounded-2xl border p-5 ${emailEnabled ? "border-emerald-500 bg-emerald-950/30" : "border-zinc-800 bg-zinc-900"}`}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-zinc-400">Aktueller Zustand</p>
              <p className={`mt-1 text-2xl font-black ${emailEnabled ? "text-emerald-300" : "text-zinc-300"}`}>
                {emailEnabled ? "E-Mail-Warnungen EIN" : "E-Mail-Warnungen AUS"}
              </p>
            </div>
            <span className={`w-fit rounded-full px-4 py-2 text-sm font-black ${emailEnabled ? "bg-emerald-500/20 text-emerald-200" : "bg-zinc-800 text-zinc-300"}`}>
              {emailEnabled ? "Aktiv" : "Deaktiviert"}
            </span>
          </div>
        </section>

        <form action={updateNotificationSettings} className="mt-6 rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
          <h2 className="text-2xl font-bold">E-Mail-Einstellungen</h2>

          <label className="mt-5 flex items-center justify-between gap-4 rounded-xl border border-zinc-700 bg-zinc-950 p-4">
            <div>
              <p className="font-bold">Warnmails erlauben</p>
              <p className="mt-1 text-sm text-zinc-400">Ausgeschaltet bedeutet: Es werden garantiert keine Warnmails versendet.</p>
            </div>
            <input
              name="email_enabled"
              type="checkbox"
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <label className="flex items-center gap-3 rounded-xl border border-zinc-700 bg-zinc-950 p-4">
              <input name="offline_alerts" type="checkbox" defaultChecked={settings?.offline_alerts ?? true} className="h-5 w-5 accent-emerald-500" />
              <span><b>Gerät offline</b><br/><small className="text-zinc-400">ESP32 oder Wetterstation</small></span>
            </label>
            <label className="flex items-center gap-3 rounded-xl border border-zinc-700 bg-zinc-950 p-4">
              <input name="frost_alerts" type="checkbox" defaultChecked={settings?.frost_alerts ?? true} className="h-5 w-5 accent-emerald-500" />
              <span><b>Frostgefahr</b><br/><small className="text-zinc-400">Wasser wird gesperrt</small></span>
            </label>
            <label className="flex items-center gap-3 rounded-xl border border-zinc-700 bg-zinc-950 p-4">
              ...
              <span><b>Kritische Fehler</b><br/><small className="text-zinc-400">Sensoren und Steuerung</small></span>
            </label>
          </div>
        ...
        <section className="mt-6 rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
          ...
          {!emailEnabled && (
            <p className="mt-3 text-sm text-amber-300">Aktiviere und speichere zuerst die Warnmails.</p>
          )}
        </section>

        <section className="mt-6 rounded-2xl border border-emerald-800 bg-emerald-950/20 p-5 text-emerald-100">
