...
import {
  formatTemperature,
  isRecentHeartbeat,
} from "../../lib/presentation/greenhouse-status";
...
export default async function WeatherPage() {
  ...
  const online = isRecentHeartbeat(weather?.last_seen, 180_000);
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950"><Nav /><div className="mx-auto max-w-7xl p-5 md:p-8">
      ...
      <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-slate-600">Außentemperatur</p><p className="mt-2 text-3xl font-black">{formatTemperature(weather?.temperature)}</p></div>
